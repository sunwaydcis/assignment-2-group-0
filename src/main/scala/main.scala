...
object HospitalData:
  ...
  def openFile(filePath: String): List[String] =
    try
      ...
      try
        file.getLines().to(List)   // If opening file success, read line
      finally
        file.close()
    catch
      case e: Exception =>
        println(s"Error opening file '$filePath': ${e.getMessage}")
        List.empty
  ...
  def parseDataToLazyList(data: List[String]): LazyList[HospitalData] =
    try
      ...
      val header = data.headOption.getOrElse("").split(",").map(_.trim)
...
@main def main(): Unit =
  ...
  if listHospital.nonEmpty then
    val stateAverages: Unit = listHospital
      ...
      .foreach { case (state, data) =>    // Iterate through each state and calculate averages for each category
